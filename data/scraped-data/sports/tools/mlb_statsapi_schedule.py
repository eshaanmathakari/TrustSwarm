"""Retrieve MLB schedule and scores from the public MLB Stats API."""
...
def get_schedule(target_date: date | None = None, team_id: int | None = None) -> Dict[str, Any]:
    """Fetch schedule (and results) for a specific date, optionally filtered by team."""
    use_date = target_date or date.today()
    params: Dict[str, Any] = {
        "sportId": 1,
        "date": use_date.strftime("%Y-%m-%d"),
        "hydrate": "linescore,team,stats",
    }
    if team_id:
